...
| 屏幕共享 | 自建 P2P WebRTC（计划中） |

服务器状态由服务器主动推送，不做轮询。身份（identity）在首次启动时本地生成并持久化，与桌面客户端的身份机制一致。

### 屏幕共享的限制

TeamSpeak 6 的屏幕共享传输协议未公开文档化，无法与官方客户端互通。本项目的屏幕共享走自建 P2P WebRTC 通道，**仅本 App 用户之间可见**：官方桌面客户端看不到本 App 发起的共享，本 App 也看不到桌面端发起的共享。语音不受此限制，与官方客户端完全互通。
